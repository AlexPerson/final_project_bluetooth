...
@property (weak, nonatomic) IBOutlet UILabel        *deviceDetailsLable;
 ...
 */
- (void)centralManager:(CBCentralManager *)central didFailToConnectPeripheral:(CBPeripheral *)selected error:(NSError *)error
{
    NSLog(@"Failed to connect to %@. (%@)", selected, [error localizedDescription]);
    [self cleanup];
}
...
- (void)cleanup
{
    ...
    if (self.selectedPeripheral.services != nil) {
                ...
                for (CBCharacteristic *characteristic in service.characteristics) {
                    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:TRANSFER_CHARACTERISTIC_UUID]]) {
                        if (characteristic.isNotifying) {
                            ...
                            [self.selectedPeripheral setNotifyValue:NO forCharacteristic:characteristic];
